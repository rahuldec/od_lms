...
@api.get("/admin/module-batch-overview")
async def module_batch_overview(_=Depends(require_admin)):
    """
    Return, for every batch:
      - which modules are currently assigned (is_active=true)
      - for each assigned module, what fraction of trainees have completed it
        (i.e. have at least one lesson_progress row with watched=true whose
         lesson_id starts with that module name, used as a prefix match)

    Also returns a flat list of all distinct module names ever assigned.
    """
    async with httpx.AsyncClient(timeout=30) as cx:
        # 1. All batches
        rb = await cx.get(f"{REST}/batches?select=*&order=created_at.desc", headers=ADMIN_HEADERS)
        batches = rb.json() if rb.status_code == 200 else []

        # 2. All active batch_module_assignments
        rma = await cx.get(
            f"{REST}/batch_module_assignments?is_active=eq.true&select=*",
            headers=ADMIN_HEADERS,
        )
        all_assignments = rma.json() if rma.status_code == 200 else []

        # 3. All trainees (need batch_id + id)
        rt = await cx.get(f"{REST}/trainees?select=id,name,batch_id", headers=ADMIN_HEADERS)
        all_trainees = rt.json() if rt.status_code == 200 else []

        # 4. All lesson_progress rows where watched=true
        rlp = await cx.get(
            f"{REST}/lesson_progress?watched=eq.true&select=trainee_id,lesson_id",
            headers=ADMIN_HEADERS,
        )
        all_progress = rlp.json() if rlp.status_code == 200 else []

    # Build lookup: trainee_id -> set of lesson_ids completed
    trainee_lessons: dict = {}
    for p in all_progress:
        tid = p["trainee_id"]
        trainee_lessons.setdefault(tid, set()).add(p["lesson_id"])

    # Build lookup: batch_id -> list of module_names assigned
    batch_modules: dict = {}
    for a in all_assignments:
        batch_modules.setdefault(a["batch_id"], []).append(a["module_name"])

    # Build lookup: batch_id -> list of trainees
    batch_trainees: dict = {}
    for t in all_trainees:
        if t.get("batch_id"):
            batch_trainees.setdefault(t["batch_id"], []).append(t)

    # All distinct module names (ever assigned, active or not)
    rall = await cx.get(
        f"{REST}/batch_module_assignments?select=module_name",
        headers=ADMIN_HEADERS,
    ) if False else None  # skip extra call; derive from active only for now

    all_module_names = sorted({a["module_name"] for a in all_assignments})

    # Build per-batch summary
    result = []
    for batch in batches:
        bid = batch["id"]
        modules = batch_modules.get(bid, [])
        trainees = batch_trainees.get(bid, [])
        total_trainees = len(trainees)

        module_stats = []
        for mod_name in modules:
            # A trainee "completed" a module if they have ≥1 watched lesson
            # whose lesson_id contains the module name (case-insensitive prefix/substring match)
            mod_lower = mod_name.lower().replace(" ", "_").replace("-", "_")
            completed_count = 0
            for t in trainees:
                lessons = trainee_lessons.get(t["id"], set())
                done = any(
                    mod_lower in lid.lower().replace(" ", "_").replace("-", "_")
                    for lid in lessons
                )
                if done:
                    completed_count += 1
            ...
            module_stats.append({
                ...
                "completion_pct": round(completed_count / total_trainees * 100) if total_trainees else 0,
            })
